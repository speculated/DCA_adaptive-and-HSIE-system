...
def load_voca_embs(voca_path, embs_path):
    # load list(self.id2word), dict(self.word2id), list(self.counts)
    voca = Vocabulary.load(voca_path)
    # load embedding
    embs = np.load(embs_path)

    # check if sizes are matched
    # word: (492408, 300) entity: (274474, 300)
    if embs.shape[0] == voca.size() - 1:
        # compress embs to form 1 * 300
...
def bin_make_equal_len(lists, max, fill_in=0, to_right=True):
    lens = [len(l) for l in lists]
    max_len = max
    if to_right:
        eq_lists = [l + [fill_in] * (max_len - len(l)) for l in lists]

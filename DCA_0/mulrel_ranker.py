...
import numpy as np
from DCA_0.local_ctx_multi_att_ranker import LocalCtxMultiAttRanker
...
class MulRelRanker(LocalCtxMultiAttRanker):
    ...
    def __init__(self, config):
        ...
        self.type_emb = torch.nn.Parameter(torch.randn([4, 5])) # trainable parameter
    ...
    def forward(self, token_ids, tok_mask, entity_ids, entity_mask, p_e_m, mtype, etype, ment_ids, ment_mask, desc_ids, desc_mask, gold=None,
        ...
        # context_emb.shape(len(mention), emb_dims, len(list(lctx_ids + rctx_ids)))
        # context_len: list(lctx_ids + rctx_ids) https://blog.csdn.net/sunny_xsc1994/article/details/82969867
        # context_cnn.shape(len(mention), 1, 64)
        context_cnn = F.max_pool1d(self.cnn(context_emb.permute(0, 2, 1)), context_len-2).permute(0, 2, 1)
        # desc_emb: desc_emb.shape (n_ments*n_cands, 500, emb_dims)
        # desc_cnn.shape(len(mention), 8, 64)
        desc_cnn = F.max_pool1d(self.cnn(desc_emb.permute(0, 2, 1))-(1-desc_mask.float())*1e10, desc_len-2).view(n_ments, n_cands, -1)

        # torch.sum: according row, * represent product, context_cnn*desc_cnn.shape(len(mention), 8, 64)
        # sim: cos(A, B) = A*B / (||A||2||B||2)
        ...
        for i in range(n_ments):
            ...
            # print('ent_coherence: ')
            # -- hongbin
            ent_coherence, selected_ents = self.mulit_compute_coherence(cumulative_entity_ids, entity_ids[indx], entity_mask[indx],
                                                   self.entity2entity_score_mat_diag,
                                                   self.tok_top_n4ent, isWord=False)
            # print('kng_coherence: ')
            kng_coherence, selected_words = self.mulit_compute_coherence(cumulative_knowledge_ids, entity_ids[indx], entity_mask[indx],
                                                   self.knowledge2entity_score_mat_diag,
    ...
    def mulit_compute_coherence(self, cumulative_ids, entity_ids, entity_mask, score_att_mat_diag, window_size, isWord=False):
        ...
        # ent_tok_att_scores, _ = torch.max(ent2ent_att_scores, dim=0)
        if isWord:
            _, ent_tok_att_scores = self.knowledge2entity_encoder(cumulative_entity_vecs, entity_vecs, entity_mask.view(n_entities, 1))
        else:
            _, ent_tok_att_scores = self.entity2entity_encoder(cumulative_entity_vecs, entity_vecs, entity_mask.view(n_entities, 1))

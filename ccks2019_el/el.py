...
random.seed(211130)  # set random seed for cpu
np.random.seed(211130)
...
if not os.path.exists('data/all_chars_me.json'):
    ...
    json.dump([id2char, char2id], open('data/all_chars_me.json', 'w'))
else:
...
if not os.path.exists('data/random_order_train.json'):
    ...
    json.dump(random_order, open('data/random_order_train.json', 'w'), indent=4)
else:
...
for k, v in subjects.items():
    for i, j in list(v.items()):
        if j < 2:
            del v[i]
    if v:
        _ = set(list(v.keys())) & set(kb2id.get(k, []))
        if _:
...
class Attention(Layer):
    ...
    def mask(self, x, mask, mode='mul'):
        if mask is None:
            return x
        for _ in list(range(K.ndim(x) - K.ndim(mask))):
...
# x1 = Attention(8, 16)([x1, x1, x1, x1_mask, x1_mask])
x2 = Attention(8, 16)([x2, x2, x2, x2_mask, x2_mask])
...
def test(test_data):
    ...
    for d in tqdm(iter(test_data)):
        ...
        F.write(json.dumps(d, ensure_ascii=False, cls=NpEncoder).encode('utf-8') + '\n')
        res += json.dumps(d, ensure_ascii=False, cls=NpEncoder).encode('utf-8') + '\n'
    F.close()
    return d
